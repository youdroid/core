"""Class to manage the services for a single platform."""
...
class ServicePlatform:
    ...
    async def async_setup_entry(self, config_entry: config_entries.ConfigEntry) -> bool:
        ...
        # Store it so that we can save config entry ID in service registry
    ...
    async def _async_setup_platform(
        ...
        with async_start_setup(hass, [full_name]):
            try:
                ...
                # Block till all services are done
                while self._tasks:
    ...
    async def _async_add_service(
        ...
        def remove_service_cb() -> None:
            ...
            # if this is the last domain platform for platform_name.
            if len(domain_platforms) > 1:
                return
    ...
    async def _async_register_service(
        ...
        await self._set_service_description(service.service_description)
        self.hass.services.async_register(
            self.domain, service.service_name, handle_service, schema
        )

    async def _set_service_description(
        self, service_description: ServiceDescription
    ) -> None:
